  ...
  return (
    <div className="min-h-screen w-full bg-twa-bg text-twa-text dark:bg-stone-950 dark:text-stone-100 overflow-x-hidden antialiased">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 sm:py-10 space-y-8">
        {/* Intro Hero Banner */}
        <section className="bg-gradient-to-br from-stone-900 via-tbilisi-slate to-stone-900 text-stone-100 rounded-3xl p-6 sm:p-8 shadow-md relative overflow-hidden">
          <div className="absolute -right-8 -bottom-8 w-48 h-48 bg-terracotta/20 rounded-full blur-2xl pointer-events-none" />
          <div className="relative z-10 space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-terracotta text-white uppercase tracking-wider">
              ✨ Interactive Catalog
            </div>
                ...
                >
                  ...
                  <div className="relative w-full h-48 bg-stone-200 dark:bg-stone-800 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10">
                      <span className="bg-terracotta text-white text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full shadow-xs tracking-wider">
                        {route.durationCategory}
                      </span>
                      <span className="bg-stone-950/90 text-stone-50 text-[10px] font-bold uppercase px-2.5 py-1 rounded-full border border-white/20">
                        {accessibilityText}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {route.vibes.map((vibe) => (
                        <span
                          key={vibe}
                          className="bg-stone-100 dark:bg-stone-800 text-stone-800 dark:text-stone-200 text-[11px] font-semibold px-2.5 py-0.5 rounded-md border border-stone-300/80 dark:border-stone-700/80"
                        >
                          #{vibe}
                        </span>
                      ))}
                    ...
                    <div className="pt-3 border-t border-stone-200 dark:border-stone-800 flex items-center justify-between text-xs font-semibold text-stone-700 dark:text-stone-300">
                      ...
                      <span className="text-terracotta dark:text-orange-400 group-hover:translate-x-1 transition-transform flex items-center gap-1 font-bold">
                        View ➔
                      </span>
                    </div>
                  </div>
