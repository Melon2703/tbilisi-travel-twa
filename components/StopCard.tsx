  ...
  return (
    ...
    >
      <div className="bg-[#1C1A17] rounded-2xl border border-[#3A342D] overflow-hidden flex flex-col h-full max-w-2xl mx-auto shadow-2xl w-full">
        ...
        {stop.imageUrl && (
          <div className="relative h-48 w-full shrink-0 bg-[#161412] overflow-hidden">
            <Image
              src={stop.imageUrl}
              alt={stop.name}
              fill
            ...
            <span className="absolute top-3 left-3 backdrop-blur-md bg-black/40 bg-slate-900/80 text-xs text-[#F4F1EA] text-white px-3 py-1 rounded-full font-medium z-20 flex items-center gap-1.5 border border-white/10 shadow-xs">
            ...
            <span className="absolute top-3 right-3 backdrop-blur-md bg-black/40 bg-slate-900/80 text-xs text-[#F4F1EA] text-white px-3 py-1 rounded-full font-medium z-20 flex items-center gap-1.5 border border-white/10 shadow-xs">
              <svg className="w-3.5 h-3.5 text-[#D96B43]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Scrollable Card Body (Middle) */}
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto max-h-[calc(100dvh-180px)] flex-1 scrollbar-none">
          ...
          <div className="flex items-start justify-between gap-2 min-w-0">
            <div className="min-w-0 flex-1">
              ...
              <h3 className="text-xl font-bold text-[#F4F1EA] tracking-wide mt-2 mb-1 leading-snug break-words min-w-0">
                {stop.name}
              </h3>
            </div>
            {stop.bestTimeOfDay && (
              <span className="shrink-0 text-[11px] font-medium bg-[#23201C] text-[#A69F95] px-2.5 py-1 rounded-full border border-white/5">
                ⏱️ {stop.bestTimeOfDay}
              </span>
            )}
          </div>

          {/* Dual Map Deep Link Pills (Secondary Placement Below Location Title) */}
          <div className="flex items-center gap-2 my-2" data-testid="map-pills-row">
            {MAP_PILLS.map((provider) => (
              <a
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 min-h-[48px] min-w-[48px] py-2 px-3 rounded-lg border border-white/5 bg-[#23201C] hover:bg-[#2E2A24] text-xs font-medium text-[#A69F95] transition-colors shadow-2xs"
              >
                <span>📍</span>
          ...
          {/* Olya's Tip Box */}
          <div className="bg-[#23201C] rounded-2xl p-4 my-3 border border-[#3A342D] space-y-1.5 shadow-2xs">
          ...
          {stop.photoSpot && (
            <div
              ...
              className="bg-[#23201C] rounded-2xl p-4 my-3 border border-[#3A342D] space-y-1 shadow-2xs"
            >
              <div className="flex items-center gap-1.5 text-[#D96B43] font-semibold text-xs uppercase tracking-wider">
                <svg className="w-4 h-4 text-[#D96B43] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-xs sm:text-sm text-[#F4F1EA] leading-normal font-medium opacity-90">
                {stop.photoSpot}
              </p>
            </div>
          )}
